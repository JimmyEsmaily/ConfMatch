...
for i=1:size(Cohs,2)
    ...
    for si=1:NumSub
        ...
        MeanVal_L(si,i) = mean(tmpdata(sBin, 5));
    end
...
for i=1:size(Cohs,2)
    ...
    for si=1:NumSub
        sBin = find(tmpdata(:,end) == si);
        MeanVal_H(si,i) = mean(tmpdata(sBin, 5));
    end
...
MyTable = table(Subs,Cohs,RT,Condition);

glme = fitglme(MyTable,...
    'RT ~ 1 + Condition + Cohs + (1|Subs)',...
