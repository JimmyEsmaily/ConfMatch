...
AnTime = dsearchn(time',[-500:4:600]');
XBin = time(AnTime);
XBin
StimDuration = dsearchn(time',[0:4:500]');



MeanLagAll = squeeze(mean(I,2,'omitnan'));
High_Indx = find(Allinfo_S(:,7)==1);
Low_Indx = find(Allinfo_S(:,7)==2);

TE_Single = mean(MeanLagAll(:,AnTime), 2);


figure, hold on
I_H = mean(MeanLagAll(High_Indx,AnTime));
I_L = mean(MeanLagAll(Low_Indx,AnTime));
...
plot([1,2],[0.07 0.07],'k-','LineWidth',2);
    text(1.3,0.075,'***','FontSize',30);
