function [Fitness,t, history,AgentConf,choice,time] = Imodel(Vars0,Cohs,ConfParams)
global info;
global Confss;
% global WsIs;
% global Bound;
...
time = time+normrnd(ND,.1,length(time),1);
% ACC = choice;
...
AACOnf = .6308;
Fitness = abs((ACC1-.84))+ abs(AACOnf-mean(AgentConf));
...
%         FinalConf=((tmpconf))/.33;(max(tmpconf));
%         FinalConf = 1.12-1.01./exp(6.2*tmpconf-.19);
%         FinalConf = 1.11-1.02./exp(6.1*tmpconf-.01);
